...
def test_create_user_without_firstname_fails():
    _, response = _make_profile(first_name=False)
    assert response.status_code == 400, response.content
...
def test_create_user_without_lastname_fails():
    _, response = _make_profile(last_name=False)
...
def test_create_user_without_username_fails():
    _, response = _make_profile(username=False)
...
def test_create_user_without_password_fails():
    _, response = _make_profile(password=False)
    assert response.status_code == 400, response.content
    data = response.json()
...
def test_edit_user_via_endpoint():
    access_token, response = _make_retrieve_profile()
    ...
    response = client.put(f"/api/profile/{user_id}", payload, format="json")
...
def test_edit_user_partial_update():
    ...
    response = client.patch(f"/api/profile/{user_id}", payload, format="json")
...
@pytest.mark.django_db
def test_retrieve_profile_by_id():
    access_token, response = _make_retrieve_profile()
    ...
    client = APIClient()
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {access_token}")
    response = client.get(f"/api/profile/{user_id}", format="json")
...
class TestDeleteUser(TransactionTestCase):
    ...
    def test_delete_user_via_endpoint(self):
        ...
        response = client.delete(f"/api/profile/{user_id}", format="json")
        assert response.status_code == 204

        payload, response = _make_profile(is_superuser=True)
